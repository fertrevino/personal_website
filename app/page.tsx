...
import { InteractiveShell } from "@/app/components/InteractiveShell";
          ...
          <div className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
            <TerminalWindow title="~/about.md">
              <p className="text-sm text-muted">
                <span className="text-accent">#</span> {about.tagline}
              </p>
              <h2 className="mt-3 text-2xl font-semibold leading-snug sm:text-3xl">
                {about.heading}
              </h2>
              <p className="mt-4 leading-relaxed text-muted">{about.body}</p>
            </TerminalWindow>
            ...
            <div className="flex flex-col gap-6">
              <TerminalWindow title="~/now.sh">
                ...
                <p className="mt-3 font-semibold">{now.role}</p>
                <p className="mt-2 text-sm text-muted">{now.description}</p>
                <div className="mt-4 flex flex-wrap gap-2 text-xs text-muted">
                  {now.tags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded border border-stroke px-2 py-0.5"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <TerminalWindow title="~/contact.sh">
                ...
                <p className="mt-2 font-semibold">{contact.heading}</p>
